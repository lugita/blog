...
<div id="comments" class="comments-area">
		...
		if (alia_option('alia_comment_website') && !is_user_logged_in()) {
				$fields['author'] = '<div class="comment_details_wrapper col8"><div class="row"><div class="col4"><input id="author" placeholder="' . esc_attr__( 'Name', 'alia' ) .
				    ( $req ? '*' : '' ) . '" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) .
				    '" size="30"' . $aria_req . ' /></div>';

				$fields['email'] =  '<div class="col4"><input id="email" placeholder="' . esc_attr__( 'Email', 'alia' ) .
				    ( $req ? '*' : '' ) . '" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) .
				    '" size="30"' . $aria_req . ' /></div>';

				$fields['url'] =  '<div class="col4"><input id="url" placeholder="' . esc_attr__( 'Website', 'alia' ) .
		...
		$user_avatar = alia_filter_lazyload_images(get_avatar( "", 45 ));
		...
		if ( is_user_logged_in() ):
		    ...
		    if ( ($current_user instanceof WP_User) ) {
		        $user_avatar =  alia_filter_lazyload_images(get_avatar( $current_user->user_email, 45 ));
		    }
	    endif;
	...
	if ( have_comments() ) : ?>
		...
		<ol class="comment-list">
			<?php
				echo alia_filter_lazyload_images(wp_list_comments( array(
					'avatar_size' => 45,
					'style'       => 'ol',
					'short_ping'  => true,
