...
<article id="post-<?php the_ID(); ?>" <?php post_class($post_classes); ?>>
	...
	<?php endif ?>
		...
		if ( ! is_single(get_the_ID()) ) {
			...
			if ( is_array(get_post_gallery('', false)) && array_key_exists('ids', get_post_gallery('', false)) ){
			...
			} else if (strpos( get_post_gallery(), "wp-block-gallery" )) {
				preg_match_all( "/blocks-gallery-item/i", get_post_gallery(), $gallery_images_numbers );
				$total_gallery_items = count($gallery_images_numbers[0]);
			}
			...
			if ( has_post_thumbnail() && ("wide" != $alia_content_width || $total_gallery_items < 3) ):
				...
				if ( is_array(get_post_gallery('', false)) && array_key_exists('ids', get_post_gallery('', false)) ) {
							...
							echo '<div class="entry-gallery row gallery_items_'.count($gallery_images_ids).'">';
										...
										foreach ($gallery_images_ids as $id) {
											...
											if (alia_option('alia_lazyload_image_banner', 0) == 1) {
												$src = 'class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src=';
											}
					...
					echo '<figure class="post_banner">';
					echo alia_filter_lazyload_images(get_post_gallery(get_the_ID(), false));
					echo '</figure>';
				};
			endif;
		};
